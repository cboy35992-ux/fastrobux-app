...
$("languageAutoDetect").checked=s.language?.autoDetect!==false;$("orders").innerHTML=d.orders.map(o=>`<button class="order-row" data-order="${esc(o.orderNumber)}"><div><b>${esc(o.orderNumber)}</b><span>${esc(o.displayName)} (@${esc(o.username)})</span></div><div><b>${Number(o.amount).toLocaleString()} Robux</b><span>₱${Number(o.totalPayment).toFixed(2)}</span></div><span class="status-pill">${esc(o.status)}</span></button>`).join("")||'<p class="muted">No matching orders.</p>';document.querySelectorAll("[data-order]").forEach(b=>b.onclick=()=>openOrder(b.dataset.order))}async function openOrder(n){selected=window.selected=await api(`/api/admin/orders/${encodeURIComponent(n)}`);$("selectedPanel").classList.remove("hidden");$("selectedNumber").textContent=selected.orderNumber;$("selectedStatus").textContent=selected.status;$("adminReceipt").href=`/api/admin/orders/${encodeURIComponent(n)}/receipt`;
const hasReceipt=Boolean(selected.hasReceipt);
$("adminReceipt").hidden=!hasReceipt;
$("adminReceipt").textContent=hasReceipt?"Open Receipt Photo":"No Receipt Photo";
$("adminReceipt").onclick=async e=>{e.preventDefault();if(!hasReceipt)return;try{const r=await fetch($("adminReceipt").href,{headers:{"x-admin-key":key},cache:"no-store"});const type=r.headers.get("content-type")||"";if(!r.ok||!type.startsWith("image/")){const d=await r.json().catch(()=>({}));throw new Error(d.error||"Receipt image is unavailable.")}const u=URL.createObjectURL(await r.blob());window.open(u,"_blank","noopener");setTimeout(()=>URL.revokeObjectURL(u),60000)}catch(error){alert(error.message)}};
const proofType=hasReceipt?"Receipt Photo":"Reference Number";
const referenceValue=selected.referenceNumber||"Not provided";
$("selectedDetails").innerHTML=`<div><span>Roblox</span><b>${esc(selected.displayName)} (@${esc(selected.username)})</b></div><div><span>Method</span><b>${esc(selected.method)}</b></div><div><span>Amount</span><b>${Number(selected.amount).toLocaleString()} Robux</b></div><div><span>Total</span><b>₱${Number(selected.totalPayment).toFixed(2)}</b></div><div><span>Sender</span><b>${esc(selected.senderName)}</b></div><div><span>Payment Proof</span><b>${esc(proofType)}</b></div><div><span>Reference</span><b>${esc(referenceValue)}</b></div>${selected.gamePassUrl?`<div><span>Buyer Game Pass</span><b><a href="${esc(selected.gamePassUrl)}" target="_blank" rel="noopener">Open Game Pass — ${Number(selected.gamePassPrice).toLocaleString()} Robux</a></b></div>`:""}`;$("adminMessages").innerHTML=selected.messages.map(m=>`<div class="chat-message ${esc(m.sender)}"><b>${esc(m.sender)}</b><p>${esc(m.text)}</p>${m.imageUrl?`<button class="proof-image-button proof-loading" data-proof-id="${Number(m.id)}"><div class="proof-loader">Loading secure proof…</div><img data-admin-proof-image="${Number(m.id)}" alt="${esc(m.imageCaption||"Order proof image")}"><span>${esc(m.imageCaption||"Open delivery proof")}</span></button>`:""}<small>${new Date(m.created_at).toLocaleString()}</small></div>`).join("");
bindAdminProofs();
...
async function loadAdminProof(id,img,button){
  try{
    const response=await fetch(`/api/admin/order-message-images/${id}`,{headers:{"x-admin-key":key},cache:"no-store"});
    ...
    img.src=url;button.classList.remove("proof-loading");
    button.onclick=()=>window.open(url,"_blank");
  }catch{button.classList.add("proof-error")}
}
function bindAdminProofs(){
  document.querySelectorAll("[data-admin-proof-image]").forEach(img=>{
    ...
    if(button&&!img.src)loadAdminProof(id,img,button);
  });
